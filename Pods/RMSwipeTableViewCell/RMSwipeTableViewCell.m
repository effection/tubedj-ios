...
- (void)awakeFromNib
{
	[super awakeFromNib];
	// We need to set the contentView's background colour, otherwise the sides are clear on the swipe and animations
	//JH [self.contentView setBackgroundColor:[UIColor whiteColor]];
	UIPanGestureRecognizer *panGestureRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanGesture:)];
	[panGestureRecognizer setDelegate:self];
...
-(BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    if ([panGestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && self.revealDirection != RMSwipeTableViewCellRevealDirectionNone) {
		...
		BOOL insideTouchArea = NO;
		if(self.revealDirection == RMSwipeTableViewCellRevealDirectionLeft) {
			if(location.x < self.touchThreshold) insideTouchArea = YES;
		} else if(self.revealDirection == RMSwipeTableViewCellRevealDirectionRight ||self.revealDirection == RMSwipeTableViewCellRevealDirectionBoth) {
			if(self.superview.bounds.size.width - self.touchThreshold < location.x) insideTouchArea = YES;
		}
		//NSLog(@"Allowed: %@ - %f",  (fabs(translation.x) / fabs(translation.y) > 1 && insideTouchArea) ? @"YES" : @"NO", location.x);
        return (fabs(translation.x) / fabs(translation.y) > 1 && insideTouchArea) ? YES : NO;
    } else {
        return NO;
    }
}
