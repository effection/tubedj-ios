...
const int JH_INTERSTITIAL_STEPS = 99;

@interface JHSideMenu ()
...
- (void)addAnimation:(NSString *)path view:(UIView *)view startValue:(double)startValue endValue:(double)endValue
{
    AccelerationAnimation *animation = [AccelerationAnimation animationWithKeyPath:path
                                                                 ...
                                                                 interstitialSteps:JH_INTERSTITIAL_STEPS];
...
{
	UIView *header = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 100, 50)];
...
{
	JHSideMenuCell *cell = [((NSArray*)_items[indexPath.section]) objectAtIndex:indexPath.row];
	cell.horizontalOffset = self.horizontalOffset;
	//JHSideMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	JHSideMenuCell *item = _items[indexPath.section][indexPath.row];
    if (item.action)
